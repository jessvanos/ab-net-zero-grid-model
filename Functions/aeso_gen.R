...
# Filter cogen data by year
cogendata <- sub_samp %>%
  mutate(YEAR=as.numeric(year(date)))%>%
  filter(Plant_Type == "COGEN",
         YEAR >= 2022,
         YEAR <= 2022)%>%
  subset(.,select=c('date','YEAR','Plant_Type','AESO_Name','Capacity','gen','Cap_Fac'))

cogendata %>%
  filter(Blackfalds (BFD1))

windows(10,8)
ggplot(cogendata) +
  geom_area(aes(x=date,y=gen), na.rm = TRUE) +
  facet_wrap(~AESO_Name) 
 
ggplot(cogendata,aes(x=Cap_Fac,y=AESO_Name,fill=stat(x))) +
  geom_density_ridges_gradient()+
  
  scale_fill_viridis(option = "D")+
  legend.title = element_text("Capacity Factor")+
  theme_bw() 


ggplot(cogendata,aes(x=Cap_Fac,y=AESO_Name,fill=Cap_Fac)) +
  geom_density_ridges(scale = 1,bins = 20,           # Overlap among histograms
                      stat = "binline")
